...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description="Run LLM persona inference.")
parser.add_argument("--country", type=str, help="Specific country to run (e.g., 'USA', 'Japan')")
parser.add_argument("--iteration", type=int, default=1, help="Iteration number for this country")
...
if args.country:
    if args.country in regions_by_country:
        # Only keep the specific country requested
        selected_regions = {args.country: regions_by_country[args.country]}
        print(f"🌍 Running inference for country: {args.country}")
    else:
        print(f"❌ Error: Country '{args.country}' not found in dictionary.")
        exit(1)
else:
    # Fallback to all countries if no parameter is provided
    selected_regions = regions_by_country
    print("🌍 Running inference for ALL countries.")
...
for age_label, age_range in age_groups.items():
    for gender in genders:
        for education in education_levels:
            for country, regions in selected_regions.items():
...
country_safe = args.country.replace(" ", "_").lower()
country_suffix = f"{country_safe}_iter{args.iteration}"
results_file = f"ipip_responses_{country_suffix}.csv"
means_file = f"ipip_means_{country_suffix}.csv"
...
trait_means.to_csv(means_file, index=False)
